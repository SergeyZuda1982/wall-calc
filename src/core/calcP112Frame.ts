...
export function calcFrameRowPositions(
  ...
  // 30.07.2026 (по прямому ответу пользователя, реальный случай — Г-контур
  // 29.73м², шаг 600мм) — пользователь заметил, что между последними двумя
  // рядами получился разрыв заметно больше шага (930мм вместо 600мм). Дело
  // в том, что код без проверки сдвигал последний ряд к дальней стене,
  // оставляя ровно CLOSE_GAP_MM=250мм зазора — но если натуральный зазор
  // до стены был почти целым шагом (не пара см, а сотни мм), такой сдвиг
  // "съедал" гораздо больше, чем несколько см, и раздувал предыдущий пролёт.
  // Правило пользователя: сдвиг последнего ряда допустим, ТОЛЬКО если он не
  // создаёт разрыв от предыдущего ряда больше шага.
  //
  // При 2+ рядах это правило математически ВСЕГДА запрещает сдвиг: ряды до
  // сжатия стоят строго через stepMm, поэтому
  //   (кандидат - предыдущий) = stepMm + (gapToWall - CLOSE_GAP_MM),
  // а мы уже внутри условия gapToWall > CLOSE_GAP_MM ⇒ второе слагаемое
  // всегда положительное ⇒ результат всегда БОЛЬШЕ stepMm. Поэтому при 2+
  // рядах последний просто никогда не двигаем — оставляем сетку как есть
  // (зазор до стены остаётся, отдельный ряд ради него всё равно не ставим).
  if (positions.length >= 2) return positions

  // Единственный ряд — сдвигать можно, если это не уводит его дальше
  // одного шага от стены начала раскладки (0).
  const last = positions[0]
  const gapToWall = spanMm - last
  if (gapToWall > CLOSE_GAP_MM) {
    const candidate = spanMm - CLOSE_GAP_MM
    if (candidate <= stepMm) positions[0] = candidate
  }
  return positions
}
