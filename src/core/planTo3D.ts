...
export function wallThicknessMm(line: PlanLine): number {
  ...
  return getLineVisual(line.type, line.spec.material, line.spec.subtype, line.spec.gapMm, line.spec.layers).thicknessMm
}
 ...
 * на кляймерах) и для неподдержанных подтипов — упрощённая равномерная сетка
 * с шагом spec.step без учёта проёмов; известное упрощение, документировано
 * здесь и там же, где аналогичное для материала.
 *
 * ⚠️ 05.09.2026: в проекте существовал ошибочный подтип 'ps125' (профиля
 * такого не бывает — реальные стоечные профили Кнауф только 50×50/75×50/
 * 100×50, направляющий 60×27, подтверждено Сергеем) — убран из таксономии
 * и отсюда полностью, не переносить обратно.
 */
