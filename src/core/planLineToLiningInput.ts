 ...
 * - spec.subtype === 'frame_ps50'/'frame_ps75' + layers===1 → С625
 * - spec.subtype === 'frame_ps50'/'frame_ps75' + layers===2 → С626
 ...
 * - нет frame_ps100 в дереве материалов wall_lining (только 50/75) —
 *   если понадобится ПС100 для облицовки, сначала расширить constructionTaxonomy.
...
export function resolveLiningType(subtype: string | undefined, layers: 1 | 2 | undefined): LiningType | null {
  ...
  if (subtype === 'frame_ps50' || subtype === 'frame_ps75') return layers === 2 ? 'c626' : 'c625'
...
export function resolveLiningProfileType(subtype: string | undefined): ProfileType | null {
  ...
  if (subtype === 'frame_ps50' || subtype === 'frame_ps75') return subtype === 'frame_ps50' ? 'ps50' : 'ps75'
  return null
}
