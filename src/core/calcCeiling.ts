 ...
 * идёт профиль. Это неверно: лист крепится саморезами по всей плоскости,
 * свободный поворот "как выгоднее по материалу" физически невозможен на
 * монтаже.
 *
 * 19.07.2026, УТОЧНЕНИЕ (репорт пользователя, прямым текстом на реальном
 * объекте): предыдущая версия этого фикса (тем же днём) детерминировала
 * ориентацию по bearingAlongLength НАПРЯМУЮ (длинная сторона листа вдоль
 * НЕСУЩЕГО) — это оказалось перепутано местами. Правильно наоборот: лист
 * ложится длинной стороной вдоль ОСНОВНОГО профиля (Осн. ПП, тот, что чаще
 * с шагом ~400-600мм) — торец листа при этом обязан попадать на НЕСУЩИЙ
 * профиль (тот, что реже, шаг b, обычно ~2500мм+) для опоры обрезного края.
 * Несущий, соответственно, служит только точкой разбежки/снэпа торцевых
 * швов (см. calcCeilingSheetRects ниже), а не направлением самого листа.
 * То же самое соотношение (шаг "несущего" — только для снэпа шва, не для
 * направления) уже было верно реализовано в раскрое контура с плана
 * (calcPolygonSheetLayout, параметр bearingStepMm) — сверено и совпадает.
 *
 * Теперь ориентация по-прежнему детерминирована bearingAlongLength (та же
 * переменная, что и у сетки каркаса calcCeilingGrid), но лист идёт вдоль
 * длинной стороны ОСНОВНОГО — то есть ПРОТИВОПОЛОЖНОЙ оси относительно
 * несущего: bearingAlongLength=true (несущий вдоль length, дефолт) → лист
 * вдоль width (ортогонально несущему, то есть вдоль основного).
 */
