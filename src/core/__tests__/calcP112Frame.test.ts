...
describe('calcFrameRowPositions', () => {
  ...
  it('30.07.2026: при 2+ рядах сдвиг последнего ряда больше не срабатывает никогда (математически)', () => {
    // span=4000, step=900: регулярные 900,1800,2700,3600 — зазор до стены 400мм (>250) —
    // кандидат на сдвиг 4000-250=3750, разрыв от предыдущего (2700) = 1050мм > шага 900мм
    // → НЕ двигаем (30.07.2026, правило по ответу пользователя)
    const positions = calcFrameRowPositions(4000, 900)
    expect(positions).toEqual([900, 1800, 2700, 3600])
  })

  it('30.07.2026: правило пользователя на чистом примере — зазор до стены остаётся, не раздувает соседний пролёт', () => {
    // step=600: натуральная сетка 600,1200,1800,2400,3000 (span=3400) —
    // зазор до стены 400мм (>250мм). Раньше код подвигал 3000 → 3150,
    // разрыв от 2400 вырастал до 750мм (> шага 600мм) — ровно тот же
    // механизм, что дал разрыв 930мм на реальном Г-контуре пользователя.
    // Теперь — не двигаем, зазор до стены остаётся 400мм.
    const positions = calcFrameRowPositions(3400, 600)
    expect(positions).toEqual([600, 1200, 1800, 2400, 3000])
    expect(3400 - positions[positions.length - 1]).toBe(400)
  })

  it('30.07.2026: при 2+ рядах сдвиг математически ВСЕГДА создал бы разрыв больше шага — проверка на наборе span/step', () => {
    // Ряды до сжатия стоят строго через stepMm, поэтому
    // (кандидат − предыдущий ряд) = stepMm + (gapToWall − CLOSE_GAP_MM), а
    // внутри условия сжатия gapToWall > CLOSE_GAP_MM всегда ⇒ результат
    // всегда БОЛЬШЕ stepMm ⇒ при 2+ рядах последний ряд теперь не двигается
    // вообще ни при каких span/step (в отличие от одиночного ряда — см. тесты
    // ниже, где сдвиг иногда всё же проходит).
    for (const [span, step] of [[4000, 900], [2830, 600], [10000, 500], [3333, 777]]) {
      const positions = calcFrameRowPositions(span, step)
      if (positions.length < 2) continue
      const expectedLast = positions[positions.length - 2] + step
      expect(positions[positions.length - 1]).toBe(expectedLast)
    }
  })

  it('одиночный ряд с дефолтным отступом (=шагу) тоже никогда не сжимается — та же математика', () => {
    // span=1200, step=900: единственный ряд на 900, зазор до стены 300мм
    // (>250), кандидат 1200-250=950 > шага 900 → не двигаем.
    const positions = calcFrameRowPositions(1200, 900)
    expect(positions).toEqual([900])
  })

  it('одиночный ряд с МАЛЫМ пользовательским отступом первого ряда — сжатие всё ещё может сработать', () => {
    // wallOffsetMm=100 (меньше шага) — тогда для span=700 остаётся ровно один
    // ряд на 100, зазор до стены 600мм (>250), кандидат 700-250=450 <= шага
    // 600 → сдвигаем. Показывает, что механизм сжатия не сломан насовсем —
    // просто при дефолтном отступе (=шагу) условие никогда не выполняется.
    const positions = calcFrameRowPositions(700, 600, { mode: 'user', wallOffsetMm: 100 })
    expect(positions).toEqual([450])
  })
