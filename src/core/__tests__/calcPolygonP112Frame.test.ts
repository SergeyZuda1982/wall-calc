...
describe('calcPolygonP112Frame — стена не в крайнем углу → сетка всё равно покрывает весь контур (фикс 12.07.2026)', () => {
  const lShape: Point2D[] = [
    { x: 0, y: 0 }, { x: 6000, y: 0 }, { x: 6000, y: 4000 },
    { x: 4000, y: 4000 }, { x: 4000, y: 6000 }, { x: 0, y: 6000 },
  ]
  ...
  it('несущие ряды есть и с отрицательной, и с положительной стороны U', () => {
    const poly = calcPolygonP112Frame(lShape, [], startSide, 1000, 500, 80, 'user')
    expect(poly.bearingRows.some(r => r.pos < 0)).toBe(true)
    expect(poly.bearingRows.some(r => r.pos > 0)).toBe(true)
  })

  it('часть контура "позади" стены больше не остаётся без крабов/подвесов', () => {
    const poly = calcPolygonP112Frame(lShape, [], startSide, 1000, 500, 80, 'user')
    // Большая комната лежит целиком в U<0 относительно этой стены — если бы
    // отрицательная сторона не считалась, там не было бы ни одного краба.
    expect(poly.crabPoints.some(p => p.x < 0)).toBe(true)
  })

  it('предупреждение больше не утверждает, что часть "не попадёт в сетку"', () => {
    const poly = calcPolygonP112Frame(lShape, [], startSide, 1000, 500, 80, 'user')
    expect(poly.warnings.some(w => w.includes('НЕ попадёт'))).toBe(false)
  })
})
