...
export const BUILTIN_WORK_STAGE_TEMPLATES: WorkStageTemplate[] = [
  ...
  {
    // 15.09.2026 (по просьбе Сергея) — «Черновой» потолок: голый
    // конструктив (монолит/металл/дерево), каркаса и обшивки нет вообще —
    // только обеспыливание и покраска по месту, как в реальной «Ведомости
    // отделки» на объекте в Ростове для технических помещений.
    id: 'ceiling_rough',
    label: 'Черновой потолок (обеспыливание + покраска)',
    context: 'ceiling',
    ceilingMaterial: ['rough'],
    steps: steps(
      ['Обеспыливание', 'Покраска', 'Готово'],
      { 'Покраска': { materialKind: 'paint' } },
    ),
  },
  {
    // 15.09.2026 (по просьбе Сергея) — готовые системы (Армстронг/
    // Реечный/Грильято/Кубота, Натяжной): после монтажа не следует НИКАКИХ
    // дальнейших работ — ни малярных, ни ГКЛ, просто "готово / не готово".
    id: 'ceiling_turnkey',
    label: 'Готовая система (Армстронг/Грильято/Кубота/Натяжной)',
    context: 'ceiling',
    ceilingMaterial: ['suspended', 'stretch'],
    steps: steps(['Готово'], {}),
  },
]
