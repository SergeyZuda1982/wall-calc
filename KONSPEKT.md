## wall-calc — конспект проекта (актуальный на 67a9524, 27.06.2026)
...
Несколько чатов могут параллельно работать над одним репо. **Перед любой правкой кода — сначала `git fetch origin` / `git log origin/main -5`, сверить с локальной копией, при расхождении подтянуть актуальное (`git reset --hard origin/main`).** Не полагаться на код "из памяти" истории чата. Последний известный коммит: **`67a9524`**.
...
### План объекта (FloorPlan.tsx) — текущее состояние

**Типы линий (PlanLineType):**
- `wall_new` — Перегородка (красный)
- `wall_lining` — Облицовка (синий)
- `wall_existing` — Сущ. стена (серый)
- `ceiling` — Потолок (фиолетовый)
- `floor` — Пол (коричневый)

**Режимы (Mode):**
- `draw` — рисование: клик→старт, клик→финиш, цепочка линий
- `select` — drag линии целиком или за концевую точку (белые кружки)
- `erase` — **мультиудаление** (клавиша R): клик по линиям помечает их (✕, полупрозрачность), Delete/R/кнопка — удалить все отмеченные, Esc — отмена
- `contour` — выделить линии → Замкнуть → площадь по формуле Гаусса
- `scale` — два клика → вводишь реальную длину в мм

**Фичи:**
- ⊾ 90° — кнопка-тогл (или Shift): снап к 0°/45°/90°/135°
- // Параллельная — копия выбранной линии со смещением в мм
- Снап к концевым точкам (SNAP_PX=18)
- Адаптивная ширина холста (ResizeObserver)
- Touch поддержка: onTap, onTouchMove, touchAction:none
- Панель выбранной линии появляется при клике в любом режиме (кроме erase/contour)
- Удаление одиночной линии: кнопка 🗑 в панели или Delete на ПК

**Архитектурная деталь — фикс Stage click:**
`lineWasClickedRef` (useRef) — устанавливается в `handleLinePointerDown` и проверяется в `handleStageClick`. Предотвращает сброс `selectedId` когда click всплывает от Group до Stage.

**Store (useProjectStore):**
```
floorPlan: { scaleMmPerPx, lines: PlanLine[], contours: PlanContour[] }
методы: addPlanLine, updatePlanLine, removePlanLine, clearFloorPlan
         addContour, removeContour, updateContour, setFloorPlanScale
миграция: contours ?? [] при rehydrate
```
...
**4. PDF-экспорт раскроя, монетизация/бэкенд**

**5. Импорт геометрии из PDF (отдельный экран)**

Концепция: "калька" поверх архитектурного плана. Пользователь сам читает план, приложение берёт только геометрию.

Схема работы:
- Загрузить PDF → рендер как фоновое изображение (`pdfjs-dist`)
- Пользователь задаёт масштаб: клик по известному отрезку → вводит его длину в мм
- Кликает начало/конец каждой стены → приложение считает длину в мм
- Отмечает проёмы (два клика = левый/правый край) → pos + width
- Кнопка "создать перегородку/облицовку" → переход в обычную форму с заполненными `length`, `openings`
- Все остальные параметры (тип профиля, слои ГКЛ и т.д.) пользователь вводит сам как обычно

Технически: react-konva уже в стеке, добавить только `pdfjs-dist`. Отдельный экран "Импорт плана".
